...
if('serviceWorker'in navigator){let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(!refreshing){refreshing=true;location.reload()}});navigator.serviceWorker.register('./sw.js').then(registration=>{registration.update();setInterval(()=>registration.update(),60*60*1000)}).catch(()=>{})}
...
let authStore=JSON.parse(localStorage.getItem(authStorageKey)||'{"users":[]}'),authMode=authStore.users.length?'login':'setup';
...
function setAuthSession(user){sessionStorage.setItem(authSessionKey,JSON.stringify({email:user.email,name:user.name,issuedAt:Date.now()}));localStorage.setItem('cornerstone-last-email',user.email);$('.profile strong').textContent=user.name;$('.profile small').textContent=user.role||'Collaborator';$('.avatar').textContent=initials(user.name);$('#authGate').hidden=true}
...
function showAuth(){const setup=authMode==='setup';$('#authEyebrow').textContent=setup?'SECURE ACCOUNT SETUP':'WELCOME BACK';$('#authTitle').textContent=setup?'Create your owner account':'Sign in to Cornerstone';$('#authCopy').textContent=setup?'Set up the first account for this Cornerstone installation.':'Sign in to restore the account and workspace information saved on this device.';$('#authNameWrap').hidden=!setup;$('#authSubmit').textContent=setup?'Create account':'Sign in';$('#authSwitch').textContent=setup?'Already have an account? Sign in':'Need to initialize this device? Create owner account';$('#authPassword').autocomplete=setup?'new-password':'current-password';$('#authEmail').value=localStorage.getItem('cornerstone-last-email')||'';$('#authPassword').value='';$('#authError').textContent='';$('#authGate').hidden=false}
async function submitAuth(){
  ...
  if(authMode==='setup'){if(!name||password.length<12){$('#authError').textContent='Enter your name and a password of at least 12 characters.';$('#authSubmit').disabled=false;$('#authSubmit').textContent='Create account';return}if(authStore.users.length){$('#authError').textContent='This device is already initialized. Choose Sign in instead.';$('#authSubmit').disabled=false;$('#authSubmit').textContent='Create account';return}const salt=crypto.getRandomValues(new Uint8Array(32)),digest=await passwordDigest(password,salt),user={id:crypto.randomUUID(),name:name.replace(/[<>]/g,''),email,role:'Owner',salt:authB64(salt),digest:authB64(digest),createdAt:new Date().toISOString()};authStore.users.push(user);localStorage.setItem(authStorageKey,JSON.stringify(authStore));authMode='login';setAuthSession(user);showToast('Owner account created','You are securely signed in.')}else{const user=authStore.users.find(u=>u.email===email);let valid=false;if(user){const salt=Uint8Array.from(atob(user.salt),c=>c.charCodeAt(0)),actual=await passwordDigest(password,salt),expected=Uint8Array.from(atob(user.digest),c=>c.charCodeAt(0));valid=constantTimeEqual(actual,expected)}if(valid){setAuthSession(user);showToast('Welcome back',`Signed in as ${user.name}.`)}else $('#authError').textContent=authStore.users.length?'Email or password is incorrect.':'No account is saved on this device. Ask an owner or admin for an invitation.'}
  $('#authSubmit').disabled=false;$('#authSubmit').textContent=authMode==='setup'?'Create account':'Sign in';$('#authPassword').value='';
}
$('#authSubmit').onclick=submitAuth;$('#authSwitch').onclick=()=>{authMode=authMode==='setup'?'login':'setup';showAuth()};$('#authPassword').addEventListener('keydown',e=>{if(e.key==='Enter')submitAuth()});
